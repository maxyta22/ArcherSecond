...
void APlayerCharacter::MakeStrike(float StrikeDistance, float MinAngle, float MaxAngle)
{
	...
	UKismetSystemLibrary::SphereTraceMultiForObjects(GetWorld(), StartTrace, EndTrace, 20, ObjectTypes, true, ActorsToIgnore, EDrawDebugTrace::ForDuration, OutHits, true, FLinearColor::Red, FLinearColor::Green, 0.5f);
			...
			if (HitResult.GetActor() != nullptr && HitResult.GetComponent() != nullptr && !IgnoreActorsDamage.Contains(HitResult.GetActor()))
			{
				...
				if (Pawn)
				{
					if (HitResult.GetComponent()->ComponentHasTag("WeakPoint"))
					{
						...
						Pawn->OnHit(GetActorForwardVector(), HitResult, WeaponType, Charged);
